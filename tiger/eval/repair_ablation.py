...
def run_repair_ablations(noisy_df: pd.DataFrame, enc: ClipEncoder, schema: Schema,
                         ...
                         cfg: dict, root: Path, generator=None, independent=None,
                         vlm_judge=None, sample_size: int | None = 20,
                         fusion=None) -> dict:
    """Repair-side ablations.

    `independent` is the encoder-family cross-check (SigLIP by default);
    `vlm_judge` is a multimodal LLM judge (Gemini). They are separate
    parameters because they answer different questions and, when both are
    supplied, get separate ablation rows.

    E13: these used to be one parameter named `vlm_judge` into which the CLI
    passed whichever verifier it had built. Both notebooks run `--independent`,
    so every committed "No VLM Judge" row actually ablated SigLIP and no VLM was
    involved in any reported number. The row is now named for what it ablates.
    """
    ...
    rep_full, rep_full_report = repair_mod.run_repair_cycle(
        ...
        max_passes=2, independent=independent, generator=generator, fusion=fusion)
    ...
    # Config 3: No Independent Verifier
    print("3/5: Running 'No Independent Verifier'...")
    rep_no_iv, rep_no_iv_report = repair_mod.run_repair_cycle(
        noisy_sample, enc, schema, thr, loo_stats, vcal, trained_model, cfg_full, root,
        max_passes=2, independent=None, generator=generator, fusion=fusion)
    results["no_independent"], _c = _evaluate_run(rep_no_iv_report, rep_no_iv, "no_independent")
    v2t_cases += _c
    ...
    rep_no_gen, rep_no_gen_report = repair_mod.run_repair_cycle(
        noisy_sample, enc, schema, thr, loo_stats, vcal, trained_model, cfg_full, root,
        max_passes=2, independent=independent, generator=None, fusion=fusion)
    ...
    # Config 6: VLM Judge instead of the encoder cross-check (D16/D17).
    # Runs only when one is supplied. The encoder verifier cannot reject a T2V
    # swap on similarity grounds -- the candidate was selected to win exactly
    # that comparison -- so the question of whether an identity-aware judge can
    # is a different question, and it gets its own row rather than replacing
    # the reported configuration. Expect this row to be slow: the judge is
    # rate-limited, one API call per repair (LRU-cached).
    if vlm_judge is not None:
        print("6/6: Running 'VLM Judge (identity-aware)'...")
        rep_vlm, rep_vlm_report = repair_mod.run_repair_cycle(
            noisy_sample, enc, schema, thr, loo_stats, vcal, trained_model,
            copy.deepcopy(cfg), root, max_passes=2, independent=vlm_judge,
            generator=generator, fusion=fusion)
        results["vlm_judge"], _c = _evaluate_run(rep_vlm_report, rep_vlm, "vlm_judge")
        v2t_cases += _c
...
def format_repair_ablations(results: dict) -> str:
    ...
    friendly_names = {
        ...
        "no_independent": "No Independent Verifier",
        "vlm_judge": "VLM Judge (identity-aware)",
        "no_gen": "No Generative Fallback",
        "no_gamma": "No Gamma Gate (γ=0)",
    }
    
    order = ["no_arbiter", "no_independent", "no_gen", "no_gamma", "vlm_judge", "full"]
    ...
    if "full" in results and "no_independent" in results:
        delta_acc = results["full"]["color_accuracy"] - results["no_independent"]["color_accuracy"]
        lines.append(f"  • Independent verifier (second encoder family): "
                     f"{delta_acc:+.1%} restoration accuracy")
    if "vlm_judge" in results and "full" in results:
        d_acc = results["vlm_judge"]["color_accuracy"] - results["full"]["color_accuracy"]
        d_rep = results["vlm_judge"]["repaired"] - results["full"]["repaired"]
        lines.append(f"  • VLM judge vs second encoder: {d_acc:+.1%} restoration accuracy, "
                     f"{d_rep:+d} repairs (it rejects what the encoder cannot -- D16/D17)")
